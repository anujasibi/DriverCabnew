    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(requestCode==PGALLERY) {
            if (data != null) {
                ...
                uri=data.getData();
                filePath = getRealPathFromURIPath(uri, Noc.this);
                try {
                    Bitmap bitmap = MediaStore.Images.Media.getBitmap(this.getContentResolver(), contentURI);
                    String path = saveImage(bitmap);
                    Toast.makeText(Noc.this, "Image Saved!", Toast.LENGTH_SHORT).show();
                    immm.setImageBitmap(bitmap);

                } catch (IOException e) {
                    e.printStackTrace();
                    Toast.makeText(Noc.this, "Failed!", Toast.LENGTH_SHORT).show();
                }
            }

        }
        if(requestCode == PCAMERA){
            Toast.makeText(Noc.this,"elbjuugv",Toast.LENGTH_SHORT).show();
...
//        Log.d("url","mmm"+filePath);
        ...
        if (filePath == null){
            Toast.makeText(Noc.this,"Please Upload Image",Toast.LENGTH_SHORT).show();
        }
        if (filePath != null) {
            File immm = new File(filePath);
            sessionManager.setNOC(filePath);
            ...
            MultipartBody.Part part1 = MultipartBody.Part.createFormData("noc", immm.getName(), photob);


            //Create request body with text description and text media type
            //
            Call<Result> call = uploadAPI.uploadNoc(part1,"Token "+ Global.user_token);
            call.enqueue(new Callback<Result>() {
                @Override
                public void onResponse(Call<Result> call, Response<Result> response) {
                    String result = response.body().getSuccess();
                    Toast.makeText(Noc.this, result, Toast.LENGTH_LONG).show();
                    if (result.equals("success")) {
                        if(sessionManager.getMake().equals("")||sessionManager.getModel().equals("")||sessionManager.getYear().equals("")||sessionManager.getColor().equals("")||sessionManager.getPlate().equals("")||sessionManager.getPermit().equals("")||sessionManager.getTourist().equals("")||sessionManager.getInsurance().equals("")||sessionManager.getFitness().equals("")||sessionManager.getRC().equals("")||sessionManager.getPho().equals("")){
                            Intent intent = new Intent(Noc.this, vehicledocument.class);
                            startActivity(intent);
                            Animatoo.animateSlideLeft(Noc.this);
                        }
                        if(!(sessionManager.getMake().equals("")||sessionManager.getModel().equals("")||sessionManager.getYear().equals("")||sessionManager.getColor().equals("")||sessionManager.getPlate().equals("")||sessionManager.getPermit().equals("")||sessionManager.getTourist().equals("")||sessionManager.getInsurance().equals("")||sessionManager.getFitness().equals("")||sessionManager.getRC().equals("")||sessionManager.getPho().equals(""))) {
                            Intent intent = new Intent(Noc.this, Approval.class);
                            startActivity(intent);
                            Animatoo.animateSlideLeft(Noc.this);
                        }
                    } else {
                ...
                public void onFailure(Call<Result> call, Throwable t) {
                    Toast.makeText(Noc.this, "Failed Upload" + t.getMessage(), Toast.LENGTH_LONG).show();
                }
            });
        }
    }
